...
function mergeRawMetadata(lead: Lead, contactPageUrl?: string, status?: string, email?: string | null) {
  const existing =
...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updatePayload: Record<string, unknown> = {
      raw_metadata: mergeRawMetadata(currentLead, emailResult.contactPageUrl, safeEmail ? "completed" : emailResult.status, safeEmail),
    };
    ...
    if (emailResult.status === "error") {
      return NextResponse.json({
        ...(updatedLead as Lead),
        success: false,
        error: "Email research failed. Please try again.",
        message: "Email research failed. Please try again.",
